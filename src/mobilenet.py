...
dataset = params.get_dataset(image_size=TARGET_SHAPE, map_fn=MobileNetModel.preprocess_input)
PRETRAIN_TOTAL_STEPS = PRETRAIN_EPOCHS * len(dataset.get_train())
basename = 'mobilenet_' + dataset.name + '_d' + str(params.dimensions) + '_m' + str(params.margin) + '_s' + str(params.epochs * 100) + '_' + params.loss
...
if inference_model_file.exists():
    ...
    save_vectors(projection_vectors, projection_labels, 'mobilenet_' + dataset.name + '_vectors')
    project_embeddings(projection_vectors, projection_labels, 'mobilenet_' + dataset.name)

    print('Computing UKBench vectors...')
    ukbench = UKBench(image_size=TARGET_SHAPE, map_fn=MobileNetModel.preprocess_input)
    ukbench_vectors, ukbench_labels = calc_vectors(ukbench.get_combined(), model)
    save_vectors(ukbench_vectors, ukbench_labels, 'ukbench_mobilenet_' + dataset.name + '_vectors')
    project_embeddings(ukbench_vectors, ukbench_labels, 'ukbench_mobilenet_' + dataset.name)
