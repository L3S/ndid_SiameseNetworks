...
For the ViT embeddings, follow approach shown here: https://syncedreview.com/2021/11/15/deepmind-podracer-tpu-based-rl-frameworks-deliver-exceptional-performance-at-low-cost-144/ (if there is a pretrained model available, use the ouput of the encoder stage as "embeddings" (i.e., image features) to feed into the dense layer for projecting to the desired dmensionality)
"""
...
projection_model = tf.keras.models.Sequential([
  layers.Dense(IMAGE_VECTOR_DIMENSIONS, activation=ACTIVATION_FN, input_shape=(EMBEDDING_VECTOR_DIMENSION,)) #relu on activation, max
])
