...
basename = 'vit_' + dataset.name + '_d' + str(params.dimensions) + '_m' + str(params.margin) + '_s' + str(params.epochs * 100) + '_' + params.loss
...
if inference_model_file.exists():
    ...
    save_vectors(projection_vectors, projection_labels, 'vit_' + dataset.name + '_vectors')
    project_embeddings(projection_vectors, projection_labels, 'vit_' + dataset.name)

    print('Computing UKBench vectors...')
    ukbench = UKBench(image_size=TARGET_SHAPE, map_fn=VitModel.preprocess_input)
    ukbench_vectors, ukbench_labels = calc_vectors(ukbench.get_combined(), model)
    save_vectors(ukbench_vectors, ukbench_labels, 'ukbench_vit_' + dataset.name + '_vectors')
    project_embeddings(ukbench_vectors, ukbench_labels, 'ukbench_vit_' + dataset.name)
